    ...
    if [ -e ./config.sh ]; then
        ...
        while true; do
            ./config.sh remove \
                --unattended \
                --auth "PAT" \
                --token "$(cat "${AZP_TOKEN_FILE}")" && break
...
# Prevent token variables from being sent to the agent
